const damerauLevenshtein = (string, stringtTarget) => {
  ...
  // cek apakah string TITLE, LOWER, atau UPPER
  let targetHasil = "";
  if (strOri === strOri.toLowerCase()) {
    targetHasil = target.toLowerCase();
  } else if (strOri === strOri.toUpperCase()) {
    targetHasil = target.toUpperCase();
  } else if (
  ...
  let similarity = (1 - distance / x) * 100;
