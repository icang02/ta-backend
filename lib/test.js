const preprocessing3 = (input, type = 0, text = "") => {
  ...
  let resultPro = preProInput
    .map((item) => {
      ...
      if (angkaHuruf.test(item)) return "";
      return item;
    })
  ...
  if (type == 1) {
    ...
    if (found != -1) resultPro = resultPro.slice(0, found);
  ...
  resultPro.forEach((item) => {
    ...
    if (reg.test(text)) finalResult.push(item);
  });
